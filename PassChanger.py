...
def connect_telnet(ip, port, username, password, new_password):
    try:
        ...
        if whatif_mode:
            logInfo.info(f"WHATIF: {ip}:{port} - Пароль будет изменён на: {new_password}")
        else:
            tn.read_until(b"# ")
            tn.write(b"password\n")
            ...
            logInfo.info(f"IP: {ip}:{port} - Пароль изменён на: {new_password}")

        tn.write(b"exit\n")
        tn.read_all()
        return True
    except Exception as e:
        logError.error(f"{ip}:{port} - {password} - Telnet ошибка подключения - {str(e)}")
        return False
...
def connect_ssh(ip, port, username, password, new_password):
    try:
        ...
        if whatif_mode:
            ...
            logInfo.info(f"{ip}:{port} - Пароль изменён на: {new_password}")

        client.close()
        return True
    except Exception as e:
        logError.error(f"{ip}:{port} - {password} - SSH ошибка подключения - {str(e)}")
        return False
...
def main():
    ...
    for row in sheet.iter_rows(min_row=2, values_only=True):
        ...
        print(F"Cоединение с {ip}:{port} используя пароль: {password}")
